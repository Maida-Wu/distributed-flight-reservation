...
class site_all_info(object):
    ...
    def delete(self, clientName):
        ...
        print("Reservation for %s c​ancelled." % (clientName))
        return True

    def hasRec(self, eR, targetSite):
        return self.matrix[targetSite][eR.siteIndex] >= eR.timestamp
    ...
    def MsgNeedSend(self, targetSite):
        index = self.siteindex[self.siteID]
        msgThinkNeed = [
            i for i in self.logs if self.hasRec(i, targetSite) == False
        ]
    ...
    def UpdateAfterReceive(self, T, NP, indexJ, userFlightInfo):
        ...
        msgTrueNeed = [i for i in NP if self.hasRec(i, index) == False]
        userPending = {}
